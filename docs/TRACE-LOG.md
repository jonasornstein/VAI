...
|------|-------------|-------|---------|-----------------|
| 2026-08-07 | P | Assistant | **Activity stats part of app** — `4e9c572` on `origin/master`; app serves `/vai-stats.html` + `/activity.jsonl`; nginx aliases removed in deploy configs; update-server verifies Lookup IPs; **prod deploy blocked** (sudo). Live still nginx-alias + old app (404 on :8765). Run: `sudo bash /opt/vai/deploy/update-server.sh` then `sudo bash /opt/vai/deploy/fix-nginx-https.sh` | https://vai.ornstein.work/vai-stats.html |
