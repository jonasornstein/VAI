...
|------|-------------|-------|---------|-----------------|
| 2026-08-07 | P | Assistant | **Activity stats part of app — commit + push** — `/vai-stats.html` + `/activity.jsonl` served by app from git tree; nginx static aliases removed; `update-server.sh` verifies Lookup IPs; **prod deploy status TBD** | `src/vai/server.py`, `deploy/update-server.sh`, `vai-stats.html` |
