# Review — V85 Proposal Axevalla 2026-07-18

| Field | Value |
|-------|-------|
| **Artifact** | `outbox/proposals/v85/2026-07-18-axevalla/proposal.md` |
| **Race card** | `inbox/race-cards/2026-07-18-axevalla.yaml` (ATG `V85_2026-07-18_8_5`) |
| **Mode** | Hari / random (seed 42) |
| **Reviewer** | ornstein (operator) |
| **Review type** | UC-20 / F-071 operator checklist |
| **Date** | 2026-07-15 |
| **Verdict** | **APPROVED** |

---

## Operator checklist (F-071)

- [x] 8 avdelningar ifyllda
- [x] Kostnad verifierad mot ATG-formel (1 000 × 0,50 = 500 SEK)
- [x] Inga ogiltiga hästnummer
- [x] Inga strukna hästar valda (re-check race day)
- [x] ATG UI transcribability confirmed by ornstein
- [x] Operator godkänner för ATG-inmatning
- [ ] Scratch/reserve status still current on race day (operator re-check before play — Saturday)

---

## Transcription guide (atg.se)

| Leg | Race | Mark on coupon | Type |
|-----|------|----------------|------|
| 1 | V85-5 | 1 | Spik |
| 2 | V85-6 | 5, 12 | Gardering (2) |
| 3 | V85-7 | 1, 2, 3, 5, 6 | Gardering (5) |
| 4 | V85-8 | 2, 9, 10, 11, 12 | Gardering (5) |
| 5 | V85-9 | 1, 2, 4, 5, 7, 9, 11, 12, 13, 14 | Gardering (10) |
| 6 | V85-10 | 12 | Spik |
| 7 | V85-11 | 9, 11 | Gardering (2) |
| 8 | V85-12 | 7 | Spik |

**Coupon summary:** 1 000 systemrader · 500,00 SEK · radpris 0,50 SEK · breakdown `1×2×5×5×10×1×2×1`

---

## Notes

- Approved after operator UI review (2026-07-15).
- Race-day UC-22 (scratches + ATG entry) scheduled for Saturday 2026-07-18.
- Canonical published copy: `outbox/proposals/v85/2026-07-18-axevalla/`.
